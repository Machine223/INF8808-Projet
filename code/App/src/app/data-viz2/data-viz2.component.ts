...
export class DataViz2Component implements OnInit {
  ...
  private createSvg(): void {
    ...
    const xScale = scaleLinear().domain([0,max(this.data, d => d.Gls + 8)])
      .range([0,innerWidth]);
    const yScale = scaleBand().domain(this.data.map(yValue))
      .range([0,innerHeight])
      .padding(0.4);
...
function steelblue(arg0: string, steelblue: any) {
